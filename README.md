...
- **Flashcards** — vocabulary in both directions (ES→EN and EN→ES, or mixed), always typed.
- **Conjugation** — a verb, a tense and one pronoun slot; type the form. A **Gerundio** tense
  drills the -ing form (-ando / -iendo, incl. irregulars like `durmiendo`, `leyendo`, `yendo`) —
  generated by rule plus an irregular table in `src/data/es/gerunds.js`, and auto-graded since the
  form is unambiguous.
...
Every drill is **auto-graded**: you type the answer (or pick an option) and the app computes the
verdict. Typed matching is lenient (case, accents and spacing) and nags about a missing accent
rather than failing you; multiple choice grades the click directly. `Wrong`/`Right` remain
available to override any verdict — useful when a synonym should have counted.

The **Challenging** list tracks vocabulary and verbs only; the small closed-set drills (numbers,
colors, positions, calendar) are deliberately excluded from it.

Each section ships a browsable reference, reachable from its setup screen: legends for Numbers,
Colors, Positions and Calendar, a searchable **word list** for Flashcards, and a regular
**endings table** (pronoun × -AR/-ER/-IR for all tenses plus the gerund) for Conjugation.
